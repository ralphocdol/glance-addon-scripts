...
document.addEventListener('DOMContentLoaded', async () => {
  var _window, _window$GLANCE_ADDON_, _glimpseConfig$glance, _Array$from$map, _document$querySelect;
  ...
  const glimpseBaseConfig = {
    ...
    showBangSuggest: true,
    searchSuggestEndpoint: '',
    pagesSlug: [],
    cleanupOtherPages: true,
    glimpseKey: '',
    waitForGlance: true,
    detectUrl: true,
    mobileBottomSearch: true,
    resizeOnSoftKeyboardOpen: false,
    autoClose: false,
    preserveQuery: true
  };
  ...
  glimpseConfig.glanceSearch.target = (_glimpseConfig$glance = glimpseConfig.glanceSearch.target) !== null && _glimpseConfig$glance !== void 0 ? _glimpseConfig$glance : '_blank';
  ...
  const mainPagePath = (_Array$from$map = Array.from(document.querySelectorAll('.nav a')).map(a => a.getAttribute('href'))) === null || _Array$from$map === void 0 ? void 0 : _Array$from$map[0];
  ...
  const glancePageTitle = ((_document$querySelect = document.querySelector('#page>h1')) === null || _document$querySelect === void 0 ? void 0 : _document$querySelect.innerText) || '';
  const iframeBySlug = {};
  ...
  if (glimpseConfig.showBangSuggest) {
    ...
    if (((_glimpseConfig$glance2 = glimpseConfig.glanceSearch) === null || _glimpseConfig$glance2 === void 0 ? void 0 : _glimpseConfig$glance2.bangs.length) > 0) {
      ...
      const searchBangItems = createElementFn({
        ...
        children: [...glimpseConfig.glanceSearch.bangs.map(b => ({
          tag: 'li',
          children: [{
            tag: 'span',
  ...
  const emptySearchSuggest = msg => `<span style="padding: 3px 15px; margin: 3px 0;">${msg !== null && msg !== void 0 ? msg : 'No suggestions…'}</span>`;
  const searchSuggestContainer = createElementFn({
    classes: 'glimpse-search-suggest',
    htmlContent: emptySearchSuggest(),
    style: {
      display: showSearchSuggest ? 'flex' : 'none'
    }
  });
  ...
  const widgetClasses = ['.widget-type-reddit', '.widget-type-rss', '.widget-type-monitor', '.widget-type-docker-containers', '.widget-type-videos', '.widget-type-bookmarks'].map(c => `${c}:not(.glimpsable-hidden)`).concat('.glimpsable').join(', ');
  let controller;
  let lastCallId = 0;
  const handleInput = debounce(async e => {
    ...
    const query = (e.target.value || '').replace(getBangRegExp, '').trim().toLowerCase();
    if (query.length < 1) {
    ...
    try {
      await searchScrape({
        contentElement: glanceContent,
        query,
        callId
      });
      const [suggestionResult] = await Promise.allSettled([showSearchSuggestion({
        query,
        controller
      }), ...glimpseConfig.pagesSlug.map(slug => otherPageScrape({
        slug,
        query,
        callId
      }))]);
      if ((suggestionResult === null || suggestionResult === void 0 ? void 0 : suggestionResult.status) === 'rejected') throw new Error(suggestionResult === null || suggestionResult === void 0 ? void 0 : suggestionResult.reason.message);
      ...
      if ((err === null || err === void 0 ? void 0 : err.name) !== 'AbortError') {
        ...
        (_window$showToast3 = (_window4 = window).showToast) === null || _window$showToast3 === void 0 || _window$showToast3.call(_window4, `Glimpse Error, see logs for more info.`, {
          type: 'error'
        });
  ...
  const handleKeydown = e => {
    ...
    if (e.key === 'Enter') {
      ...
      const currentBangString = (_query$match = query.match(getBangRegExp)) === null || _query$match === void 0 || (_query$match = _query$match[0]) === null || _query$match === void 0 ? void 0 : _query$match.trim();
      const currentBangObject = glimpseConfig.glanceSearch.bangs.find(b => b.shortcut === currentBangString);
      if (currentBangObject !== null && currentBangObject !== void 0 && currentBangObject.rawQuery) {
  ...
  document.addEventListener('keydown', event => {
    ...
    if (['INPUT', 'TEXTAREA', 'SELECT'].includes(activeElement.tagName) || activeElement.isContentEditable || activeElement.closest('#glimpse')) {
      return;
    }
    const hasActiveModal = !!(modalElement && getComputedStyle(modalElement).display !== 'none');
    if (glimpseConfig.glimpseKey && event.code === keyToCode(glimpseConfig.glimpseKey) && activeElement !== searchInput && !hasActiveModal) {
  ...
  async function searchScrape({
    ...
    query,
    callId,
    pageTitle = glancePageTitle
  }) {
    if (callId !== lastCallId) return;
    const columns = contentElement === null || contentElement === void 0 ? void 0 : contentElement.querySelectorAll('.page-columns');
    if (!(columns !== null && columns !== void 0 && columns.length)) return;
    await Promise.allSettled(Array.from(columns).flatMap(column => [...Array.from(column.querySelectorAll(widgetClasses)).flatMap(widget => [createWidgetResult({
      widget,
      query,
      callId,
      pageTitle,
      ...
      callId,
      pageTitle,
      listSelector: 'ul.dynamic-columns',
      itemSelector: ':scope > .monitor-site, .docker-container, .flex'
    }), createWidgetResult({
      widget,
      query,
      callId,
      ...
      callId,
      pageTitle,
      listSelector: '.cards-vertical',
      itemSelector: ':scope > .widget-content-frame'
    })]), ...Array.from(column.querySelectorAll('.glimpsable-custom')).map(widget => createWidgetResult({
      widget,
      query,
      callId,
      pageTitle,
      listSelector: '[glimpse-list]'
    })), ...Array.from(column.querySelectorAll('.glimpsable-custom-list')).map(widget => createWidgetResult({
      widget,
      query,
      callId,
      pageTitle,
  ...
  async function otherPageScrape({
    slug,
    query,
    callId
  }) {
    ...
    async function docSearch(doc, s) {
      ...
      await searchScrape({
        ...
        query,
        callId,
        pageTitle: ((_doc$querySelector = doc.querySelector('#page>h1')) === null || _doc$querySelector === void 0 ? void 0 : _doc$querySelector.innerText) || ''
      });
    }
  }
  ...
  async function showSearchSuggestion({
    query,
    controller
  }) {
    ...
    try {
      const getSuggestion = await fetch(glimpseConfig.searchSuggestEndpoint + encodeURIComponent(query), {
        signal: controller.signal
      });
      const result = await getSuggestion.json();
      if (!(result !== null && result !== void 0 && result[1].length)) return;
      ...
      (_window$showToast4 = (_window5 = window).showToast) === null || _window$showToast4 === void 0 || _window$showToast4.call(_window5, `Glimpse search suggest error, see logs for more info`, {
        type: 'error'
      });
      console.error(e);
      controller.abort();
  ...
  async function createWidgetResult({
    widget,
    query,
    callId,
    pageTitle,
    ...
    return new Promise(resolve => {
      var _widget$querySelector, _ref, _document$getElementB, _widgetContent$closes;
      if (callId !== lastCallId) return resolve();
      const headerSource = (_widget$querySelector = widget.querySelector('.widget-header > h2')) === null || _widget$querySelector === void 0 ? void 0 : _widget$querySelector.innerText;
      ...
      const resultSearch = [...ulLists].flatMap(ul => {
        ...
        return [...items].filter(el => ((el === null || el === void 0 ? void 0 : el.innerText) || '').replace(/\n/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase().includes(query));
      });
      if (!resultSearch.length) return resolve();
      const newTitle = (_ref = headerSource !== null && headerSource !== void 0 ? headerSource : (_document$getElementB = document.getElementById((_widgetContent$closes = widgetContent.closest('.widget-group-content')) === null || _widgetContent$closes === void 0 ? void 0 : _widgetContent$closes.getAttribute('aria-labelledby'))) === null || _document$getElementB === void 0 ? void 0 : _document$getElementB.innerText) !== null && _ref !== void 0 ? _ref : '';
  ...
  if (typeof window.createCustomMenuItemElement === 'function') {
    var _window$createCustomM, _window6;
    (_window$createCustomM = (_window6 = window).createCustomMenuItemElement) === null || _window$createCustomM === void 0 || _window$createCustomM.call(_window6, {
  ...
  if (customSettingsFunctions && typeof customSettingsFunctions === 'object') {
    ...
    (_customSettingsFuncti = customSettingsFunctions.createCustomSettingsItem) === null || _customSettingsFuncti === void 0 || _customSettingsFuncti.call(customSettingsFunctions, {
      ...
      contentObject: [{
        type: 'text',
        name: 'Load configuration from Path/URL',
        key: 'glimpse-configuration-url',
        ...
        type: 'text',
        name: 'Search URL',
        key: 'glanceSearch.searchUrl',
        value: storedGlimpseConfig.glanceSearch.searchUrl,
        colOffset: 1
      }, {
        type: 'text',
        name: 'Search Suggest Endpoint',
        key: 'searchSuggestEndpoint',
        value: storedGlimpseConfig.searchSuggestEndpoint,
        colOffset: 1,
        ...
        key: 'glanceSearch.placeholder',
        value: storedGlimpseConfig.glanceSearch.placeholder,
        colOffset: 2
      }, {
        type: 'dropdown',
        name: 'Search Target',
        key: 'glanceSearch.target',
        value: storedGlimpseConfig.glanceSearch.target,
        options: ['_blank', '_self', '_parent', '_top']
      }, {
        type: 'text',
        name: 'Shortcut Key',
        key: 'glimpseKey',
        value: storedGlimpseConfig.glimpseKey,
        maxLength: 1
      }, {
        type: 'toggle',
        name: 'Show Bang Suggestions',
        key: 'showBangSuggest',
        value: storedGlimpseConfig.showBangSuggest
      }, {
        type: 'toggle',
        name: 'Cleanup Other Pages',
        key: 'cleanupOtherPages',
        value: storedGlimpseConfig.cleanupOtherPages,
        tooltip: 'Cleans other page search on Glimpse close. High resource usage if false.'
      }, {
        type: 'multi-text',
        name: 'Other Page Search (Slug)',
        key: 'pagesSlug',
        value: storedGlimpseConfig.pagesSlug,
        colOffset: 1,
        ...
        key: 'waitForGlance',
        value: storedGlimpseConfig.waitForGlance
      }, {
        type: 'toggle',
        name: 'Detect URL',
        key: 'detectUrl',
        value: storedGlimpseConfig.detectUrl
      }, {
        type: 'toggle',
        name: 'Mobile Bottom Search',
        key: 'mobileBottomSearch',
        value: storedGlimpseConfig.mobileBottomSearch,
        tooltip: 'Repositions the search bar and the suggestions to the bottom in mobile view for ease of access.'
      }, {
        type: 'toggle',
        name: 'Resize On Keyboard',
        key: 'resizeOnSoftKeyboardOpen',
        value: storedGlimpseConfig.resizeOnSoftKeyboardOpen,
        tooltip: 'On most mobile browsers, when a soft keyboard is present, the page will just overlay making the entire content scrollable. This will result in disabled horizontal scroll of content near the soft keyboard. This attempts to fix that by making the content resized instead.'
      }, {
        type: 'toggle',
        name: 'Auto Close',
        key: 'autoClose',
        value: storedGlimpseConfig.autoClose
      }, {
        type: 'toggle',
        name: 'Preserve Search Query',
        key: 'preserveQuery',
        value: storedGlimpseConfig.preserveQuery,
        tooltip: 'Preserves Query on search.'
      }, {
        type: 'textarea',
        name: 'Search Bangs (json)',
        key: 'glanceSearch.bangs',
        value: JSON.stringify(storedGlimpseConfig.glanceSearch.bangs, null, 2).trim(),
        colOffset: 1
      }, {
        type: 'buttons',
        buttons: [{
        ...
        setup: () => {
          ...
          function setAndSave(config, name, value) {
            try {
              ...
              toastNotification(`Glimpse configuration successfully updated.`, {
                type: 'success'
              });
            } catch (e) {
              toastNotification(`Glimpse configuration failed to update, see logs for more info.`, {
                type: 'error'
              });
              console.error(e);
            }
          ...
          function setValuesWithConfig(config) {
            _SET_KEYED_ELEMENT_('glanceSearch.searchUrl', {
              value: config.glanceSearch.searchUrl
            });
            _SET_KEYED_ELEMENT_('searchSuggestEndpoint', {
              value: config.searchSuggestEndpoint
            });
            _SET_KEYED_ELEMENT_('glanceSearch.target', {
              value: config.glanceSearch.target
            });
            _SET_KEYED_ELEMENT_('glanceSearch.placeholder', {
              value: config.glanceSearch.placeholder
            });
            _SET_KEYED_ELEMENT_('glimpseKey', {
              value: config.glimpseKey
            });
            _SET_KEYED_ELEMENT_('showBangSuggest', {
              checked: config.showBangSuggest
            });
            _SET_KEYED_ELEMENT_('cleanupOtherPages', {
              checked: config.cleanupOtherPages
            });
            _SET_KEYED_ELEMENT_('pagesSlug', {
              value: config.pagesSlug
            });
            _SET_KEYED_ELEMENT_('waitForGlance', {
              checked: config.waitForGlance
            });
            _SET_KEYED_ELEMENT_('detectUrl', {
              checked: config.detectUrl
            });
            _SET_KEYED_ELEMENT_('mobileBottomSearch', {
              checked: config.mobileBottomSearch
            });
            _SET_KEYED_ELEMENT_('resizeOnSoftKeyboardOpen', {
              checked: config.resizeOnSoftKeyboardOpen
            });
            _SET_KEYED_ELEMENT_('autoClose', {
              checked: config.autoClose
            });
            _SET_KEYED_ELEMENT_('preserveQuery', {
              checked: config.preserveQuery
            });
            _SET_KEYED_ELEMENT_('glanceSearch.bangs', {
              value: JSON.stringify(config.glanceSearch.bangs, null, 2).trim()
            });
          }
        },
        ready: () => {
          ...
          try {
            if (target.dataset.key === 'glimpse-configuration-url') {
              if (keyEl.value && (await confirmDialog('Downloading configuration, this will replace your existing one, proceed?'))) {
                try {
                  var _window$showToast5, _window7;
                  ...
                  (_window$showToast5 = (_window7 = window).showToast) === null || _window$showToast5 === void 0 || _window$showToast5.call(_window7, 'Glimpse Configuration successfully loaded.', {
                    type: 'success'
                  });
                  setTimeout(() => location.reload(), 1000);
                } catch (err) {
                  var _window$showToast6, _window8;
                  (_window$showToast6 = (_window8 = window).showToast) === null || _window$showToast6 === void 0 || _window$showToast6.call(_window8, 'Failed to fetch configuration, see logs.', {
                    type: 'error'
                  });
                  console.error(err);
                }
            ...
            if (target.dataset.key === 'glanceSearch.searchUrl') {
              ...
              targetEl.dataset.defaultSearchUrl = keyEl === null || keyEl === void 0 ? void 0 : keyEl.value;
              setAndSave(storedGlimpseConfig, keyEl === null || keyEl === void 0 ? void 0 : keyEl.name, keyEl === null || keyEl === void 0 ? void 0 : keyEl.value);
              return;
            }
            if (target.dataset.key === 'glanceSearch.placeholder') {
              const targetEl = glimpseEl.querySelector('.glimpse-search .search-input');
              targetEl.placeholder = keyEl === null || keyEl === void 0 ? void 0 : keyEl.value;
              setAndSave(storedGlimpseConfig, keyEl === null || keyEl === void 0 ? void 0 : keyEl.name, keyEl === null || keyEl === void 0 ? void 0 : keyEl.value);
              return;
            }
            if (target.dataset.key === 'mobileBottomSearch') {
              const targetEl = glimpseEl.querySelector('.glimpse-wrapper');
              targetEl.classList.toggle('bottom-search', keyEl === null || keyEl === void 0 ? void 0 : keyEl.checked);
              setAndSave(storedGlimpseConfig, keyEl === null || keyEl === void 0 ? void 0 : keyEl.name, keyEl === null || keyEl === void 0 ? void 0 : keyEl.checked);
              return;
            }
            ...
            if (target.dataset.key === 'copy-all-json-configuration') {
              try {
                var _window$showToast7, _window9;
                ...
                (_window$showToast7 = (_window9 = window).showToast) === null || _window$showToast7 === void 0 || _window$showToast7.call(_window9, 'JSON configuration has been copied to clipboard.', {
                  type: 'success'
                });
              } catch (err) {
                var _window$showToast8, _window0;
                (_window$showToast8 = (_window0 = window).showToast) === null || _window$showToast8 === void 0 || _window$showToast8.call(_window0, 'Failed to copy JSON config, see logs', {
                  type: 'error'
                });
                console.error(err);
              }
            ...
            if (findElementByCardAndName('card-toggle', keyEl === null || keyEl === void 0 ? void 0 : keyEl.name) !== undefined) {
              saveAndReload(storedGlimpseConfig, keyEl === null || keyEl === void 0 ? void 0 : keyEl.name, keyEl === null || keyEl === void 0 ? void 0 : keyEl.checked);
            }
            if (findElementByCardAndName('card-text', keyEl === null || keyEl === void 0 ? void 0 : keyEl.name)) {
              saveAndReload(storedGlimpseConfig, keyEl === null || keyEl === void 0 ? void 0 : keyEl.name, keyEl === null || keyEl === void 0 ? void 0 : keyEl.value);
            }
            if (findElementByCardAndName('card-multi-text', keyEl === null || keyEl === void 0 ? void 0 : keyEl.name)) {
              var _keyEl$value;
              saveAndReload(storedGlimpseConfig, keyEl === null || keyEl === void 0 ? void 0 : keyEl.name, keyEl === null || keyEl === void 0 || (_keyEl$value = keyEl.value) === null || _keyEl$value === void 0 ? void 0 : _keyEl$value.split(','));
            }
            if ((keyEl === null || keyEl === void 0 ? void 0 : keyEl.name) === 'glanceSearch.bangs') {
              saveAndReload(storedGlimpseConfig, keyEl === null || keyEl === void 0 ? void 0 : keyEl.name, JSON.parse(keyEl === null || keyEl === void 0 ? void 0 : keyEl.value));
            }
            if (target.dataset.key === 'reload-page') {
              if (await confirmDialog('Reload page?')) location.reload();
            }
          } catch (e) {
            toastNotification('Error in configuration: ' + (keyEl === null || keyEl === void 0 ? void 0 : keyEl.name) + ', see logs for more info.`', {
              type: 'error'
            });
            console.error(e);
          }
