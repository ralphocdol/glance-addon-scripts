...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelectorAll('[responsive-table]').forEach((t, t_index) => {
    ...
    if (isPaginate[t_index]) {
      ...
      (_t$querySelector = t.querySelector('button.expand-toggle-button')) === null || _t$querySelector === void 0 || _t$querySelector.remove();
  ...
  function setupTable({
    ...
    if (sorted) return sortTable({
      t,
      t_index,
      body
    });
    for (const h of sortable) {
      const defaultDirection = h.hasAttribute('sort-default-asc') ? 'asc' : h.hasAttribute('sort-default-desc') ? 'desc' : 'none';
  ...
  function sortTable({
    ...
    const sorted = [...data[t_index]].sort((a, b) => {
      var _a$children$col, _a$children$col2, _b$children$col, _b$children$col2;
      const x = ((_a$children$col = a.children[col]) === null || _a$children$col === void 0 ? void 0 : _a$children$col.dataset.toSort) || ((_a$children$col2 = a.children[col]) === null || _a$children$col2 === void 0 ? void 0 : _a$children$col2.innerText.trim()) || '';
      const y = ((_b$children$col = b.children[col]) === null || _b$children$col === void 0 ? void 0 : _b$children$col.dataset.toSort) || ((_b$children$col2 = b.children[col]) === null || _b$children$col2 === void 0 ? void 0 : _b$children$col2.innerText.trim()) || '';
  ...
  function setupMobileSort({
    ...
    const directionBtn = createMobileSortBtn(colSortedDirection[t_index], t_index, () => {
      ...
      if (!isNaN(colSorted[t_index])) sortTable({
        t,
        t_index,
        body
      });
  ...
  function updatePaginationButtons(container, {
    ...
    var _paginationBtnControl;
    container.innerHTML = '';
    const totalPages = Math.ceil(data[t_index].length / dataPageLimit[t_index]);
    const current = dataPage[t_index];
    if (totalPages <= 1) return;
    (_paginationBtnControl = paginationBtnControllers[t_index]) === null || _paginationBtnControl === void 0 || _paginationBtnControl.abort();
    paginationBtnControllers[t_index] = new AbortController();
    const btnHandler = e => {
      if (!e.target.classList.contains('page-btn')) return;
      const btn = e.target.textContent;
      if (!isNaN(btn)) dataPage[t_index] = Number(btn);else if (btn === '←' && current > 1) dataPage[t_index]--;else if (btn === '→' && current < totalPages) dataPage[t_index]++;else return;
      updatePage({
        t,
        t_index,
        header,
    ...
    container.addEventListener('click', btnHandler, {
      signal: paginationBtnControllers[t_index].signal
    });
    ...
    // Page Buttons
    const pages = [];
    if (totalPages <= 7) for (let i = 1; i <= totalPages; i++) pages.push(i);else if (current <= 4) pages.push(1, 2, 3, 4, 5, '...', totalPages);else if (current >= totalPages - 3) pages.push(1, '...', totalPages - 4, totalPages - 3, totalPages - 2, totalPages - 1, totalPages);else pages.push(1, '...', current - 1, current, current + 1, '...', totalPages);
    for (const p of pages) {
      if (p === '...') addEllipsis();else container.appendChild(createBtn(p, p === current));
    }
