...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function openModal(targetElement) {
    ...
    modalContainer.classList.remove(...Object.values(ENUM_WIDTH).map(size => `modal-width-${size}`), ...Object.values(ENUM_HEIGHT).map(size => `modal-height-${size}`));
