...
(() => {
  ...
  const CURRENT_SCRIPT_URL = CURRENT_SCRIPT ? new URL(CURRENT_SCRIPT.src, window.location.origin) : null;
  const CURRENT_SCRIPT_TIMESTAMP = CURRENT_SCRIPT_URL ? CURRENT_SCRIPT_URL.searchParams.get('v') : '';
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    function newThemePropertiesManager() {
      ...
      const applyBackgroundImage = (fadeImage = true) => {
        ...
        setTimeout(() => {
          setRootVars({
            '--background-image-url-body': isValid ? `url(${themeProperties.backgroundImage}?v=${CURRENT_SCRIPT_TIMESTAMP})` : 'var(--color-background)'
          });
